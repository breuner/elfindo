...
#
# Call this script from the repo root dir in a cygwin shell. The created package can run in a
# normal Windows command-prompt and does not require cygwin to be installed on the host.
#
...
ARCHIVE_NAME="${PACKAGE_NAME}-windows.zip"
...
cp -v bin/${FIND_EXE_NAME}.exe "${PACKAGING_PATH}/" || exit 1

for i in $(ldd bin/${FIND_EXE_NAME}.exe | grep -o '\/usr\/bin\/.*dll'); do
    cp -v $i "${PACKAGING_PATH}/" || exit 1
done

cd "$PACKAGING_PATH" || exit 1

zip -v "$ARCHIVE_NAME" *.exe *.dll || exit 1

echo "ALL DONE. Your package is here: ${PACKAGING_PATH}/${ARCHIVE_NAME}"
